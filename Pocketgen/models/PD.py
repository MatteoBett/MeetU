...
def to_sdf(pred_pos, elements, mask, ligand_batch, bond_types, bond_index, edge_batch, id, target_path):
    ...
    for l in range(num_ligand):
        ...
        # Correct potential ring-related issues
        for atom in mol.GetAtoms():
            is_in_ring = atom.IsInRing()
            if not is_in_ring:  # Check if atom is incorrectly labeled
                atom.SetIsAromatic(False)
                atom.SetBoolProp("_IsInRing", False)  # Reset any ring-related flags

        # Attempt to sanitize and correct errors
        try:
            Chem.SanitizeMol(mol)
        except Chem.MolSanitizeException as e:
            print(f"Sanitization error for molecule {id + l}: {e}")
            for atom in mol.GetAtoms():
                atom.SetIsAromatic(False)  # Reset aromaticity if errors persist
            try:
                Chem.SanitizeMol(mol, Chem.SanitizeFlags.SANITIZE_ALL ^ Chem.SanitizeFlags.SANITIZE_SETAROMATICITY)
            except Exception as final_e:
                print(f"Failed to fully sanitize molecule {id + l}: {final_e}")

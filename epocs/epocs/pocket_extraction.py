...
class GetPockets:
    ...
    def run_sequence_extraction_in_parallel(self, items, num_processes=8):
        ...
        if 'shared' not in items[0]:
            items = [os.path.join(root_dir, elt[3:]) for elt in items]
        if num_processes == 1:
    ...
    def adjust_path(self, items):
        root_dir = os.path.abspath(os.path.join(__file__, "..", ".."))
        new_items = []
        for path_1, l_path in items:
            path_1 = os.path.join(root_dir, path_1[3:])
            l_path = [os.path.join(root_dir, elt[3:]) for elt in l_path]
            new_items.append((path_1, l_path))
        return new_items
    ...
    def run_pocket_extraction_in_parallel(self, items, num_processes=8):
        ...
        if 'shared' not in items[0][0]:
            items = self.adjust_path(items)
        if num_processes == 1:
            for item in tqdm.tqdm(items):
